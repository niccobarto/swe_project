...
Considerando che ogni classe DAO presente nel package ORM per funzionare deve ottenere una connessione al database, si vuole definire un meccanismo che permetta di centralizzare questo processo. La classe \texttt{DBConnection} implementa il pattern creazionale \textbf{Singleton}, utilizzato per la garanzia dell'esistenza di un'unica istanza di connessione al database.
...
Come riportato nella sezione precedente, il database di produzione non subisce nessuna interazione con queste classi. E' il database creato appositamente per il testing a ricevere le query dei metodi. Per ogni unità di test implementata vengono eseguiti due metodi dedicati all'inizializzazione e alla pulizia del database. Il medoto \textbf{setUp()} viene utilizzato per ottenere la connessione e popolare database di test oltre ad inizializzare i DAO. Il metodo \textbf{tearDown()} viene eseguito dopo ogni test e si occupa di ripulire il database e rilasciare la connessione.
